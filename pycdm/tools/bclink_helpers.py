...
from pycdm.cdm.objects import get_cdm_tables
...
class BCLinkHelpers(BashHelpers,Logger):
    ...
    def check_global_ids(self,output_directory,chunksize=10):
        ...
        data = pycdm.tools.load_csv({"ids":f"{output_directory}/global_ids.tsv"},
                                        sep='\t',
                                        chunksize=100)
    ...
    def remove_table(self,fname):
        ...
        data = pycdm.tools.load_csv({table:{'fields':[0],'file':fname}},
                                        sep='\t',
                                        chunksize=1000)
