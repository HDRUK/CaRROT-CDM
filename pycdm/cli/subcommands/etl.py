...
import pycdm
from pycdm.tools.bclink_helpers import BCLinkHelpers
from pycdm.tools.logger import _Logger as Logger
...
def _check_conf(config):
    ...
    # try:
    #     rules = pycdm.tools.load_json(config['transform']['rules'])
...
def _run_extract(config):
    ...
    files = pycdm.tools.get_files(input_folder,type='csv')
    for f in files:
...
def _find_data(d):
    ...
    subfolders = pycdm.tools.get_subfolders(root_input_folder)
    data = [
...
def _load_transform_data(data,processed_data={}):
    ...
    for _id,_data in data.items():
        ...
        _data['rules'] = pycdm.tools.load_json_delta(_data['rules'],processed_rules)
...
def _run_data(data,clean,ctx):
    ...
    inputs = pycdm.tools.get_files(input_folder,type='csv')
    ...
    except pycdm.cdm.model.PersonExists as e:
        logger.error(e)
...
@click.option('--log-file','-l',default='pycdm.log',help='specify the log file to write to')
@click.pass_context
...
def bclink(ctx,force,config_file,interactive):
    ...
    try:
        rules = pycdm.tools.load_json(config['rules'])
        destination_tables = list(rules['cdm'].keys())
    except Exception as e:
        raise BadRulesFile(e)
...
def _process_list_data(ctx):
    ...
    while True:
        ...
        if new_rules_file:
            ...
            rules = pycdm.tools.load_json_delta(rules_file,rules)
            rules_file_last_modified = os.path.getmtime(rules_file)
            re_execute = True
        else: 
            ...
            if change_in_rules:
                ...
                rules = pycdm.tools.load_json_delta(current_rules_file,rules)
                re_execute = True 
        ...
        if re_execute:
            ...
            for item in new_data:
                if isinstance(item['input'],list):
                    ...
                    inputs = pycdm.tools.get_files(input_folder,type='csv')
                filtered_rules = pycdm.tools.remove_missing_sources_from_rules(rules,inputs)
        ...
        if new_rules_file or change_in_rules:
            ...
            rules = pycdm.tools.load_json(current_rules_file)
...
def _process_dict_data(ctx):
    ...
    while True:
        ...
        subfolders = pycdm.tools.get_subfolders(input_folder)
        # if len(subfolders)>0:
        ...
        for name,path in sorted(subfolders.items(),key=lambda x: os.path.getmtime(x[1])):
            ...
            inputs = pycdm.tools.get_files(path,type='csv')
            filtered_rules = pycdm.tools.remove_missing_sources_from_rules(rules,inputs)

            if output_folder_exists:
                output_tables = [
                    os.path.splitext(os.path.basename(x))[0]
                    for x in pycdm.tools.get_files(f"{output_folder}/{name}",type='tsv')
                ]
                ...
                filtered_rules = pycdm.tools.filter_rules_by_destination_tables(filtered_rules,to_process)
...
def delete_tables(ctx):
    ...
    all_files = [
        pycdm.tools.get_files(f,type='tsv')
...
def delete_data(ctx):
    ...
    folders = pycdm.tools.get_subfolders(output_data)
    ...
    for selected_folder in selected_folders:
        files = pycdm.tools.get_files(selected_folder,type='tsv')
...
def _extract(ctx,data,rules,bclink_helpers):
    ...
    if isinstance(inputs,str):
        ...
        inputs = pycdm.tools.get_files(inputs)
        if len(inputs) == 0:
    ...
    if do_pseudonymise:        
        ...
        if not isinstance(rules,dict):
            rules = pycdm.tools.load_json(rules)
        person_id_map = pycdm.tools.get_person_ids(rules)
...
def _execute(ctx,
    ...
    if interactive and ('extract' in steps or 'transform' in steps):
        ...
        for table in tables:
            source_tables = [
                f"{data['input']}/{x}"
                for x in pycdm.tools.get_source_tables_from_rules(rules,table)
            ]
        ...
        if len(tables) == 0:
            ...
            return
        rules = pycdm.tools.filter_rules_by_destination_tables(rules,tables)
        logger.info(f'cdm tables: {tables}')
    ...
    cdm_tables = pycdm.tools.get_files(output_folder,type='tsv')
    if interactive:
        choices = []
        for x in cdm_tables:
...
def _get_table_map(table_map,destination_tables):
    ...
    if not isinstance(table_map,dict):
        table_map = pycdm.tools.load_json(table_map)
...
def manual(ctx,rules,inputs,output_folder,clean,table_map,gui_user,user,database,dry_run):
    ...
    _rules = pycdm.tools.load_json(rules)
