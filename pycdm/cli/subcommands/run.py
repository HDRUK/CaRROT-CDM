...
@click.pass_context
def test(ctx):
    _dir = os.path.dirname(os.path.abspath(pycdm.__file__))
...
def transform(inputs,config,number_of_rows_per_chunk,output_folder):
    ...
    operation_tools = pycdm.cdm.OperationTools()
...
def format(inputs,number_of_rows_per_chunk,output_folder):
    ...
    cdm = pycdm.cdm.CommonDataModel.from_existing(inputs=inputs,
...
def merge(inputs,output_folder):
    ...
    outputs = pycdm.tools.create_csv_store(output_folder=output_folder,sep='\t',write_mode='w',write_separate=False)
...
def load(inputs,config):
    ...
    config = json.loads(config)
    if 'bclink' in config:
        outputs = pycdm.io.BCLinkDataCollection(config['bclink'])
    else:
...
def format_input_data(column,operation,input):
    ...
    """
    optools = pycdm.cdm.OperationTools()
...
def ___analysis(ctx,config,analysis_names,max_workers,batch):
    ...
    inputs = config['cdm']
    inputs = pycdm.tools.load_tsv(config['cdm'],
                                      dtype=None)

    cdm = pycdm.cdm.CommonDataModel.load(inputs=inputs)
...
def map(ctx,rules,inputs,format_level,
    ...
    #if log_file == 'auto' and pycdm.params['log_file'] is None:
    if log_file == 'auto':
        log_file = f"{output_folder}{os.path.sep}logs{os.path.sep}pycdm.log"
        pycdm.params['log_file'] = log_file
    elif log_file == 'none':
        pass
    else:
        pycdm.params['log_file'] = log_file
    ...
    if tables:
        ...
        config = pycdm.tools.filter_rules_by_destination_tables(config,tables)

    if objects:
        objects = list(set(objects))
        config = pycdm.tools.filter_rules_by_object_names(config,objects)
    ...
    if any('*' in x for x in inputs):
        data_dir = os.path.dirname(pycdm.__file__)
    ...
    if db:
        ...
        if allow_missing_data:
            config = pycdm.tools.remove_missing_sources_from_rules(config,inputs)
    ...
    if isinstance(output_database,dict):
        if 'bclink' in output_database:
            outputs = pycdm.tools.create_bclink_store(bclink_settings=output_database['bclink'],
        ...
        outputs = pycdm.tools.create_csv_store(output_folder=output_folder,
                                                   sep=csv_separator,
                                                   write_separate=split_outputs,
                                                   write_mode=write_mode)
    else:
        outputs = pycdm.tools.create_sql_store()

    #build an object to store the cdm
    cdm = pycdm.cdm.CommonDataModel(name=name,
                                        inputs=inputs,
...
def run_pyconfig(ctx,rules,pyconf,inputs,objects,
    ...
    if pyconf not in available_classes:
        ctx.invoke(list_classes)
        raise KeyError(f"cannot find config {pyconf}. Run 'pycdm map py list' to see available classes.")
...
def gui(ctx):
    ...
    sg.theme_add_new('pycdm', pycdm_theme)
    sg.theme('pycdm')
    ...
    window = sg.Window('pycdm', layout, font=font)
    while True:
