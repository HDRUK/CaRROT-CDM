...
@click.option("-t","--token",help="specify the pycdm_token for accessing the CCOM website",type=str,default=None)
...
def ccom(report_id,number_of_events,output_directory,
    ...
    token = os.environ.get("pycdm_TOKEN") or token
    if token == None:
        raise MissingToken("you must use the option --token or set the environment variable pycdm_TOKEN to be able to use this functionality. I.e  export pycdm_TOKEN=12345678 ")
...
def synthetic_cdm(config):
    ...
    for destination_table_name,destination_table in order:
        ...
        obj = pycdm.cdm.get_cdm_class(destination_table_name)()
...
def mapping_rules(report):
    ...
    #date_lookup = pycdm.cdm.get_cdm_class(destination_
    ...
    def create_hde_from_value(concept,value,table,field,person_id,date_event):
        ...
        obj = pycdm.cdm.get_cdm_class(destination_table)()
    ...
    report = pycdm.tools.load_json(report)
    hdes = []
    for table in report:
